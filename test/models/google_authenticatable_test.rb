...
class GoogleAuthenticatableTest < ActiveSupport::TestCase
  ...
  test 'testing class method for finding by tmp key' do
    ...
    assert_not User.find_by(gauth_tmp: User.find(1).gauth_tmp).nil?
  end

  test 'testing token validation' do
    assert_not User.find(1).validate_token('1')
    assert_not User.find(1).validate_token(ROTP::TOTP.new(User.find(1).get_qr).at(Time.now))
